...
import { getAll, getLocation, getLocationView } from '../actions';
import AssetEdit from './assetEdit';




const MyReferralList = ({id}:{id:string}) => {
      ...
      useEffect(() => {
          const fetchData = async (elementId:string) =>{
           const rc = await getAll()
            return rc
          }
            fetchData(id).then((x)=>{
             let s:string = x
            let l:AssetList[] =  JSON.parse(s,function(key,value){
                  let t:AssetList[]= JSON.parse(value)
                return t
             })
             ...
             l.map((v,i) =>{
              ...
              //console.log('asset ->',v.Record.assetID,v.Record.name)
    ...
    <div className='col-span-4 bg-base-200'>
        {!showSearchResult && (
          <div className='bg-inherit'>
            <button className="btn btn-primary max-w-sm sm:btn-sm md:btn-md lg:btn-md xs:btn-xs" onClick={() => router.push(`/assetobj`)} >
                Create New Asset
            </button> 
          </div>
        )}
                          ...
                          return (
                            <tr key={x.assetID} className='hover'>
                              ...
                              <td>
                                  ...
                                  <button title="Asset History" className="btn   btn-link max-w-sm sm:btn-sm md:btn-md lg:btn-md xs:btn-xs" onClick={() => router.push(`/assetobj/history/${x.assetID}`)} >History</button>
